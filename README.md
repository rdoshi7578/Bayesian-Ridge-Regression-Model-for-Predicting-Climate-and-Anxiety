...
Authors: Dishi Arun, John Black, Norah O'Callaghan, Rutva Doshi, Suvan Pattanaik Biomedical Engineering, Georgia Institute of Technology
...
Goals
Joint predictive modeling. Build a multi-output model that predicts both anxiety-related ED visit rates and climate variables from a shared feature set, filling a gap in existing literature where these are modeled separately.
Regional robustness. Evaluate the model across five geographically and climatically distinct U.S. cities — San Francisco, Los Angeles, Atlanta, Chicago, and New York — representing Northern California, Southern California, the Southeast, the Midwest, and the Northeast.
Data Sources
NA-CORDEX (North American domain of the Coordinated Regional Climate Downscaling Experiment): daily climate variables (2000–2025) including max/min near-surface air temperature, surface downwelling shortwave radiation, surface pressure, precipitation rate, and near-surface wind speed, spatially extracted for the five target cities via nearest-neighbor grid lookup and aggregated to monthly means.
...
Update this section to match the actual folder layout of the repo.
...
Update the commands below to reflect how the code is actually run (script vs. notebook, dependencies, etc.).
...
A mockup GUI demonstration is available here: https://youtu.be/nc81qrzeqRc
